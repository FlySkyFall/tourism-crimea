document.addEventListener('DOMContentLoaded', () => {
  ...
  let currentRegionId = null; // Хранит ID региона, если переход с региона

  // Обработчик клика по ссылке региона
  document.querySelectorAll('.region-card a').forEach(link => {
    link.addEventListener('click', (e) => {
      currentRegionId = link.getAttribute('href').split('/').pop(); // Извлекаем ID региона
    });
  });

  const fetchTours = async (type = 'all', page = 1) => {
    if (toursList && window.location.pathname === '/') { // Выполняем только на главной странице
  ...
  // Инициализация текущих фильтров из URL только на главной странице
